...
export default function VideoPlayer({ src, title, onNextEpisode, nextEpisodeLabel, onBack, initialTime = 0, onProgress }) {
  ...
  const handleSeekMouseDown = (e) => {
    setSeeking(true);
    setSeekValue(e.nativeEvent.offsetX / e.currentTarget.offsetWidth * duration);
  };
      ...
      <div className={`vp-controls ${showControls ? 'vp-controls-visible' : 'vp-controls-hidden'}`}>
        ...
        <div className="vp-bottom-bar">
          <div className="vp-left-controls">
            ...
            <button onClick={() => skip(-SKIP_SECONDS)} className="vp-btn" aria-label="Rewind 10s">
            ...
            <button onClick={() => skip(SKIP_SECONDS)} className="vp-btn" aria-label="Forward 10s">
            ...
            <div className="vp-volume-group">
              <button onClick={toggleMute} className="vp-btn" aria-label="Mute">
                {muted || volume === 0 ? (
